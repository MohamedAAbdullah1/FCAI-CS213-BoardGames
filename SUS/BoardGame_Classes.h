...
public:
    ...
    void set_last_move(int x,int y){last_move=make_pair(x,y);}

    pair<int,int> get_last_move(){return last_move;}

    void increas_score(){score++;}
...
public:
    ...
    void run() {
        ...
        while (boardPtr->columns*boardPtr->rows+1) {
            for (int i : {0, 1}) {
                ...
                if (boardPtr->is_win(currentPlayer)) {
                    ui->display_message(currentPlayer->get_name() + " has get point!");
                    currentPlayer->increas_score();
                    continue;
                }
                
                // if (boardPtr->is_lose(currentPlayer)) {
                //     ui->display_message(players[1 - i]->get_name() + " has get point!");
                //     continue;
                // }
                 
                if (boardPtr->game_is_over()) {
                    ui->display_message(currentPlayer->get_name()+":"+to_string(currentPlayer->get_score())
                +"\n"+players[1-i]->get_name()+":"+to_string(players[1-i]->get_score()));
                    return;
                }
            }
        }
        ui->display_message(players[0]->get_name()+":"+to_string(players[0]->get_score())
                +"\n"+players[1]->get_name()+":"+to_string(players[1]->get_score()));
    }
};
...
Player<T>** UI<T>::setup_players() {
    ...
    players[0] = create_player(nameS, static_cast<T>('S'), typeS);

    string nameU = get_player_name("Player U");
    PlayerType typeU = get_player_type_choice("Player U", type_options);
    players[1] = create_player(nameU, static_cast<T>('U'), typeU);
