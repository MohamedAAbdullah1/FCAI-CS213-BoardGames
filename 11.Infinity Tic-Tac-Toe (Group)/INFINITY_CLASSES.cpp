...
Infinity_Board::Infinity_Board() : Board<char>(3, 3) {
    ...
    board.assign(rows, vector<char>(columns, blank_symbol));
    n_moves = 0;
    removals_done = 0;
    moves_queue.clear();
}
...
bool Infinity_Board::update_board(Move<char>* move) {
    ...
    real_board[x][y] = mark;
    moves_queue.emplace_back(x, y, mark);
...
Move<char>* Infinity_UI::get_move(Player<char>* player) {
    ...
    for (int r = 0; r < player->get_board_ptr()->get_rows(); ++r)
        for (int c = 0; c < player->get_board_ptr()->get_columns(); ++c)
            if (player->get_board_ptr()->get_board_matrix()[r][c] == '.') // if showing blank
