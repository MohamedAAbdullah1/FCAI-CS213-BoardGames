...
class Move {
    ...
    T symbol;
public:
    Move(int xf, int yf,int xt ,int yt , T symbol) : x1(xf), y1(yf),x2(xt) ,y2(yt) , symbol(symbol) {}

    int get_to_x() const { return x2; }
    int get_from_x() const {return x1 ;}
    int get_to_y() const { return y2; }
    int get_from_y() const {return y1;}
...
protected:
    string name;
    PlayerType type;
    T symbol;         // not strictly required for word game, kept for compatibility
    Board<T>* boardPtr;
public:
